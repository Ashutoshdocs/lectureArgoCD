# Chapter 6: ArgoCD Notifications (Email)
...
This chapter wires **email** notifications to the **chai-app** and demonstrates
two events: **app successfully deployed** and **app health degraded**.

- **Repo:** `https://github.com/Ashutoshdocs/lectureArgoCD.git`
- **App manifests path:** `Chapter06_argoCD_Notifications/applicationsets/chai-app`
...
| Application annotations | `chai-app.yaml` | Subscriptions (who gets which event) |
...
```bash
kubectl apply -f Chapter06_argoCD_Notifications/secret-smtp.yaml
```
...
```bash
kubectl apply -f Chapter06_argoCD_Notifications/argocd-notifications-cm.yaml
...
### Step 3: Deploy the chai-app with subscriptions
...
```bash
git add Chapter06_argoCD_Notifications && git commit -m "Chapter06: chai-app notifications" && git push origin main
kubectl apply -f Chapter06_argoCD_Notifications/chai-app.yaml
```
...
kubectl get all -n default -l app=chai-app
...
**1. Deployed (success).**
When chai-app finishes sync and becomes **Healthy**, the `on-deployed` trigger
fires → you get an email:
```
[ArgoCD] chai-app successfully deployed 🎉
```
...
```bash
# in Chapter06_argoCD_Notifications/applicationsets/chai-app/deployment.yaml
#   image: nginx:1.27-alpine   ->   image: nginx:v1
git commit -am "break chai-app image (demo)" && git push origin main
```
...
**Revert** to fix:
```bash
git commit -am "restore chai-app image" && git push origin main
```
...
* This demo emails on **deployed** and **degraded** for chai-app.
...
```bash
kubectl delete -f Chapter06_argoCD_Notifications/chai-app.yaml
